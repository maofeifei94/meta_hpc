...
class trainer_info():
    ...
    ec_cvae_load_newest_model=True
...
class trainer():
    ...
    def agent_env_process(self,pipe_dict):
        ...
        def RL_loop():
            ...
            env_obs = env.reset()
            # print("rl loop 1.1")
            env_obs=np.transpose(env_obs[:-2],[2,0,1])
            # print("rl loop 1.2")
            ...
            for _e in range(trainer_info.pfc_start_train_epoch):
                ...
                for i in range(hp.PPO_NUM_STEPS):
                    ...
                    next_env_obs=np.transpose(next_env_obs[:-2],[2,0,1])
                    #next iter
            ...
            for e in range(int(1e10)):
                ...
                if e%20==0:
                    ...
                    cvae.save_model("all_models/ec_cvae64_model",e)
                    cvae.save_model("all_models/ec_cvae64_model","newest")
            ...
            while 1:
                ...
                if rpm._size>=trainer_info.ec_rpm_start_train_size:
                    ...
                    if train_steps%10000==0:
                        cvae.save_model("all_models/ec_cvae64_model",train_steps)
                        cvae.save_model("all_models/ec_cvae64_model","newest")
                else:
