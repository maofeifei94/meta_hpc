...
class trainer():
    ...
    def process_env_data_to_rpm(self,x):
    ...
    def agent_env_process(self,pipe_dict):
        ...
        def RL_loop():
            ...
            env_obs = env.reset()
            # print("rl loop 1.1")
            env_obs=self.process_env_data_to_rpm(env_obs)
            # print("rl loop 1.2")
            ...
            for _e in range(trainer_info.pfc_start_train_epoch):
                ...
                for i in range(hp.PPO_NUM_STEPS):
                    ...
                    next_env_obs=self.process_env_data_to_rpm(next_env_obs)
                    #next iter
            ...
            for e in range(int(1e10)):
                ...
                if e%20==0:
                    ...
                    cvae.save_model("all_models/ec_cvae_model",e)
                    cvae.save_model("all_models/ec_cvae_model","newest")
    ...
    def train_ec_process(self,pipe_dict):
        ...
        rpm=ReplayMemory(
            [
                DataFormat("env_img",trainer_info.env_img_shape,np.uint8)
            ],max_size=trainer_info.ec_rpm_size
        )
        cvae=ConvVAE()
        if trainer_info.ec_cvae_load_newest_model:
            cvae.load_model("all_models/ec_cvae_model","newest")
            ...
            while 1:
                if rpm._size>=trainer_info.ec_rpm_start_train_size:
                    ...
                    if train_steps%10000==0:
                        cvae.save_model("all_models/ec_cvae_model",train_steps)
                        cvae.save_model("all_models/ec_cvae_model","newest")
                else:
