...
import os 
import copy
from pfc.gru_pfc_env_hpc_pred import PPO_GRU_Module
from env.env_war_frog import multi_circle_env
...
def from_env_obs_to_input_dict(env_obs_dict,reward):
    ...
    hpc_img=env_obs_dict['hpc_img']
    ask_site_norm=env_obs_dict['ask_site_norm']

    # print(ball_obs.shape,label_img.shape)

    input_dict={
        'env_img':paddle.to_tensor(np.transpose(np.expand_dims(np.concatenate([ball_obs,label_img],axis=-1),axis=[0,1]),[0,1,4,2,3]),'float32'),
        'env_vec':paddle.to_tensor(np.expand_dims(np.concatenate([ball_site_norm,[np.float32(reward)]],axis=-1),[0,1]),'float32'),
        'hpc_img':paddle.to_tensor(np.transpose(np.expand_dims(hpc_img,axis=[0,1]),[0,1,4,2,3]),'float32'),
        'hpc_vec':paddle.to_tensor(np.expand_dims(ask_site_norm,[0,1]),'float32'),
    }
    return input_dict
...
def main():
    ...
    pfc_train.load_model('all_models/pfc_model',200)
    ...
    for e in range(10000000000000000000):
        ...
        with paddle.no_grad():
            for j in range(hp.PPO_NUM_STEPS):
                ...
                pfcpred=pfc_output_dict['pfcpred']
                print(np.shape(pfcpred))
                pfcpred_img=np.reshape(pfcpred,[64,64])
                # pfcpred_img=(pfcpred_img-np.min(pfcpred_img))/(np.max(pfcpred_img)-np.min(pfcpred_img))
                pfcpred_img=(np.clip(pfcpred_img,-1,1)+1)/2
                cv2.imshow("pfcpred",cv2.resize(pfcpred_img,dsize=(320,320),interpolation=cv2.INTER_NEAREST))
                env.render()
