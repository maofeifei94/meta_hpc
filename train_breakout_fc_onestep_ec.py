...
def train():
    ...
    from ec.seq_vae_pred_fc import SeqVaePred
    from ec.hyperparam.breakout_seq_onestep_pred_fc_hp import svaepred_info
    import paddle
    from mlutils.logger import mllogger
    ...
    for data in npy_data_list:
        ...
        for key in ["state","reward","action"]:
            ...
            data_dict[key]=data_dict[key][:,:].astype(np.float16)
    ...
    def train_thread(queue_dict):
        ...
        for i in range(load_num+1 if load_num is not None else 1,1000000000000):
            ...
            loss=svaepred.train_with_warmup_onesteppred(batch_state,batch_action,pre_global_h=batch_global_h,pre_local_h=batch_local_h)
            t3=time.time()
