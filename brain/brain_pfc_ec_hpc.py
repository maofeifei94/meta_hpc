...
class FlowGenerator():
    ...
    def generate_offline_train_flow(self):
        ...
        cache=Cache(self.share_info,self.lock)
        return TaskFlow(tasklist=tasklist,cache=cache,priority='low')
    def generate_cae_train_flow(self):
        tasklist=[
            Task(filter_name='datafilter',func_name='sample_data',batch_size=brain_hyperparam.train_batch_size,seq_len=brain_hyperparam.train_seq_len,use_recent_rpm=False),
            Task(filter_name='caefilter',func_name='train_with_pred',img_save_dir="all_data/cae_img"),
            Task(filter_name='caefilter',func_name='save_model',save_interval=1000,save_dir=brain_hyperparam.cae_hyperparam.model_dir),
    ...
    def generate_ec_train_flow(self):
        tasklist=[
            Task(filter_name='datafilter',func_name='sample_data',batch_size=brain_hyperparam.train_batch_size,seq_len=brain_hyperparam.train_seq_len,use_recent_rpm=False),
            Task(filter_name='caefilter',func_name='pred',cache_data_key='data_sampledata'),
            Task(filter_name='ecfilter',func_name='train'),
            Task(filter_name='ecfilter',func_name='save_model',save_interval=1000,save_dir=brain_hyperparam.ec_info.model_dir)
        ]
        cache=Cache(self.share_info,self.lock)
    ...
    def generate_h_update_flow(self,info_dict):
        ...
        tasklist=[
            ...
            Task(filter_name='caefilter',func_name='pred',cache_data_key='data_samplehupdateseq'),
...
class EnvFilter(ProcessFilter):
    def __init__(self, queue_dict,name='envfilter') -> None:
        ...
        class cae_ec(nn.Layer):
            def __init__(self, name_scope=None, dtype="float32"):
                super().__init__(name_scope, dtype)
                self.cae=Conv_Auto_Encoder(brain_hyperparam.cae_hyperparam)
                self.ec=SeqVaePred(brain_hyperparam.ec_info)
            def forward(self,env_obs_paddle,action_env_discrete_onehot):
                cae_vec=self.cae.encode_static(env_obs_paddle).reshape([1,1,-1])
                ec_vec,pre_glo_h,pre_loc_h,inner_reward_ori=self.ec.pred_env(cae_vec.reshape([1,1,-1]),action_env_discrete_onehot.reshape([1,1,-1]))
                return cae_vec,ec_vec,pre_glo_h,pre_loc_h,inner_reward_ori

        self.cae_ec=cae_ec()
        # self.cae_ec=to_static(cae_ec(),input_spec=[
        #     InputSpec(shape=[1,3,64,64], name='env_obs_paddle'), 
        #     InputSpec(shape=[1,1,brain_hyperparam.env_action_discrete_num], name='action_env_discrete_onehot')
        #     ])
    ...
    def _env_obs_to_input_dict_and_inner_reward(self,env_obs,action_env_discrete_onehot):
        env_obs_paddle=paddle.to_tensor(env_obs,dtype='uint8')
        env_obs_paddle=paddle.cast(env_obs_paddle,'float32')/255
        env_obs_paddle=paddle.paddle.transpose(env_obs_paddle,perm=[2,0,1])
        env_obs_paddle=paddle.expand(env_obs_paddle,shape=[1,*env_obs_paddle.shape])
    ...
    def update_all_param(self,cache,**kwargs):
        ...
        self.cae_ec.cae.update_model_from_np(cache['cae_getcaeparam'])
        self.cae_ec.ec.update_model_from_np(cache['ec_getecparam'])
        cache_update={}
        return cache_update
...
class DataFilter(ProcessFilter):
    def __init__(self, queue_dict,name='datafilter') -> None:
        self.rpm=LMDBRecoder(
            data_format_list=[
                DataFormat(name='img',shape=[3,64,64],dtype=np.uint8,key_head=0),
                ...
                DataFormat(name='reward',shape=[1],dtype=np.float32,key_head=5),
    ...
    def h_update(self,cache,**kwargs):
        if cache['data_samplehupdateseq'] is None:
            ...
            self.rpm.h_update(glo_h,loc_h,index)
            cache_update={}
            return cache_update
...
class CaeFilter(ProcessFilter):
    ...
    def train_with_pred(self,cache,**kwargs):
        ...
        if data_sampledata is None:
            return {"cae_trainwithpred":None}
        else:
            batch_seq_img=data_sampledata['img']
    ...
    def pred(self,cache,**kwargs):
        data=cache[kwargs['cache_data_key']]
        if data is None:
            return {"cae_pred":None}
        else:
            batch_seq_img=data['img']
            img_cae_mean=self.cae.pred(batch_seq_img)
            cache_update={"cae_pred":img_cae_mean}
            self.train_steps+=1
            return cache_update
...
class EcFilter(ProcessFilter):
    ...
    def pred_h(self,cache,**kwargs):
        ...
        if s_history is None:
            ...
            pred_glo_h,pred_loc_h=self.ec.pred_h(s_history,a_history,pre_glo_h,pre_loc_h)
            cache_update={
                'ec_predh':{
    ...
    def train(self,cache,**kwargs):
        ...
        if data_sampledata is None:
            ...
            if 'cae_trainwithpred' in cache.keys():
                state_history=cache['cae_trainwithpred']
            elif 'cae_pred' in cache.keys():
                state_history=cache['cae_pred']
            else:
                raise KeyError("cache has no key cae_trainwithpred or cae_pred")
