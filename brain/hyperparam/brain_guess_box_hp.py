...
class ec_info:
    ...
    recon_seq_len=64
    base_dim=64
    klpred_moving_gamma=0.995
    "input dim"
    gloinfo_gauss_dim=base_dim
    state_vec_dim=env_action_discrete_num
    action_vec_dim=env_action_discrete_num

    "train param"
    train_lr=0.0001
    train_kl_loss_ratio=2e-10

    # gloinfo
    "input fc"
    gloinfo_ifc_input_dim=state_vec_dim+action_vec_dim
    gloinfo_ifc_mid_dim=base_dim
    gloinfo_ifc_mid_layers=4
    gloinfo_ifc_output_dim=base_dim
    "gru"
    gloinfo_gru_dim=base_dim
    "gauss_fc"
    gloinfo_gfc_input_dim=gloinfo_gru_dim
    gloinfo_gfc_mid_dim=base_dim
    ...
    locinfo_ifc_mid_dim=base_dim
    locinfo_ifc_mid_layers=4
    locinfo_ifc_output_dim=base_dim
    "gru"
    locinfo_gru_dim=base_dim

    #predfc
    "pred fc"
    pred_fc_input_dim=locinfo_gru_dim+gloinfo_gauss_dim+action_vec_dim
    pred_fc_mid_dim=base_dim
    pred_fc_mid_layers=6
    pred_fc_output_dim=state_vec_dim

    #klpred
    klpred_ifc_input_dim=state_vec_dim+action_vec_dim
    klpred_ifc_mid_dim=base_dim
    klpred_ifc_mid_layers=4
    klpred_ifc_output_dim=base_dim

    klpred_gru_dim=base_dim

    klpred_fc_input_dim=klpred_gru_dim+action_vec_dim
    klpred_fc_mid_dim=base_dim
    klpred_fc_mid_layers=4
    klpred_fc_output_dim=1
...
class brain_hyperparam:
    ...
    PPO_input_output_info={
        'to_static':False,
        'env_vec_dim':env_action_discrete_num+ec_info.gloinfo_gauss_dim*2+ec_info.locinfo_gru_dim,
        'obs_dim':env_action_discrete_num+ec_info.gloinfo_gauss_dim*2+ec_info.locinfo_gru_dim,
