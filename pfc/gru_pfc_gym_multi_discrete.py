...
class pfc_actor(parl.Model):
    def __init__(self, input_output_info):
        ...
        self.actor_discrete_num_list=input_output_info['actor_discrete_num_list']
        ...
        "从gru到env_action"
        self.action_net_list=[]
        for i,action_num in enumerate(self.actor_discrete_num_list):
            pre_action_num=np.sum(self.actor_discrete_num_list[:i])
            action_net=nn.Sequential(
                nn.Linear(self.gru_hid_dim+pre_action_num,128),nl_func(),
                nn.Linear(128,128),nl_func(),
                nn.Linear(128,128),nl_func(),
                nn.Linear(128,action_num)
                )
            self.action_net_list.append(action_net)
    ...
    def forward(self,input_dict,train,h_dict):
        ...
        action_dict={
            ...
            "gru_out":gru_out,
            "action_net_list":self.action_net_list
        }
...
class GRUPPO(parl.Algorithm):
    def __init__(self,model:GRU_PPO_Model):
        ...
        #     epsilon=hp.PPO_EPS,
        #     grad_clip=clip)
    def generate_action_from_actor(self,action_gru_out,action_net_list):
        "generate action"
        action_list=[]
        action_log_probs_list=[]
        pre_action_onehot_list=[]
        for i,action_net in enumerate(action_net_list):
            
            if i==0:
                action_i_logits=self.model.actor_model.squeeze_time_layer(action_gru_out,action_net)
            else:
                action_i_logits=self.model.actor_model.squeeze_time_layer(paddle.concat([action_gru_out,*pre_action_onehot_list],axis=-1),action_net)

            action_i_probs=F.softmax(action_i_logits)
            action_i_dim=action_i_probs.shape[-1]
            action_i_dist=m_Categorical(logits=action_i_probs)
            action_i=action_i_dist.sample[1]
            action_i_onehot = paddle.squeeze(F.one_hot(action_i, action_i_dim),axis=-2)
            action_i_log_probs=paddle.sum(action_i_probs*action_i_onehot,axis=-1,keepdim=True)

            action_list.append(action_i)
            action_log_probs_list.append(action_i_log_probs)
            pre_action_onehot_list.append(action_i_onehot.detach())
        return action_list,action_log_probs_list
    def generate_probs_of_old_action(self,action_gru_out,action_net_list,old_action_list):
        actor_discrete_num_list=self.model.actor_model.actor_discrete_num_list

        old_action_onehot_list=[]
        for i,old_action_i in enumerate(old_action_list):
            old_action_i_onehot=paddle.squeeze(F.one_hot(old_action_i[i:i+1],actor_discrete_num_list[i]),axis=-2)
            old_action_onehot_list.append(old_action_i_onehot)

        old_action_new_log_probs_list=[]
        for i,action_net in enumerate(action_net_list):
            if i==0:
                action_i_logits=self.model.actor_model.squeeze_time_layer(action_gru_out,action_net)
            else:
                action_i_logits=self.model.actor_model.squeeze_time_layer(paddle.concat([action_gru_out,*old_action_onehot_list[:i]],axis=-1),action_net)

            action_i_probs=F.softmax(action_i_logits)
            old_action_i_new_log_probs=paddle.sum(action_i_probs*old_action_onehot_list[i],axis=-1,keepdim=True)

            old_action_new_log_probs_list.append(old_action_i_new_log_probs)
        return old_action_new_log_probs_list
    def learn_actor(self,train_dict):
        ...
        return action_continuous_loss.numpy(), action_continuous_dist_entropy.numpy(),action_discrete_loss.numpy(),action_discrete_dist_entropy.numpy()
    ...
    def sample(self, input_dict,train,h_dict):
        ...
        "discrete action"
        action_gru_out=action_dict["gru_out"]
        action_net_list=action_dict["action_net_list"]

        "generate action"
        action_list,action_log_probs_list=self.generate_action_from_actor(action_gru_out,action_net_list)
        sample_dict={
            "value":value,
            "action_list":action_list,
            "action_log_probs_list":action_log_probs_list,
        }
        return sample_dict
    ...
    def learn(self, next_value, gamma, gae_lambda, ppo_epoch, num_mini_batch,
        ...
        for e in range(ppo_epoch):
            ...
            for sample_dict in data_generator:
                ...
                if t%hp.PPO_CRITIC_TRAIN_TIME==0:
        ...
        action_continuous_loss_epoch /= (num_updates/hp.PPO_CRITIC_TRAIN_TIME)
        action_continuous_dist_entropy_epoch /= (num_updates/hp.PPO_CRITIC_TRAIN_TIME)
        action_discrete_loss_epoch /= (num_updates/hp.PPO_CRITIC_TRAIN_TIME)
        action_discrete_dist_entropy_epoch /= (num_updates/hp.PPO_CRITIC_TRAIN_TIME)
