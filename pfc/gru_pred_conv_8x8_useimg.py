...
class GruPredHid(nn.Layer):
    ...
    def train_history(self,s_history,a_history):
        ...
        pred_img=s_his[:,0:1]
        decoder_h=paddle.reshape(encoder_hid[:,-1:,:],[1,_b,self.gru_dim])
        # decoder_output_list=[]
        pred_img_list=[]
        for _i in range(_t):
            sa=paddle.concat([pred_img,a_his[:,_i:_i+1]],axis=-1)
            sa_feature=self.btd_layer(self.decoder_input_fc,sa)
            decoder_output,decoder_h=self.decoder_gru(sa_feature,decoder_h)
            # decoder_output_list.append(decoder_output)
            pred_img=self.btd_layer(self.decoder_output_fc,decoder_output)
            pred_img_list.append(pred_img)
        decoder_output=paddle.concat(pred_img_list,axis=1)
