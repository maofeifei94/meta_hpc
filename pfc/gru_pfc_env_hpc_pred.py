...
class pfc_actor(parl.Model):
    def __init__(self, input_output_info):
        ...
        "从gru1到pred"
        self.gru1_to_pred=nn.Sequential(
            Conv_Decoder_net()
            # nn.Linear(gru1_hid_dim,np.prod(input_output_info['hpc_img_shape']))
        )
    ...
    def forward(self,input_dict,train,h_dict):
        ...
        "pred"
        _b,_t,_d=gru1_out.shape
        pfcpred=self.squeeze_time_layer(paddle.reshape(gru1_out,[_b,_t,1,16,16]),self.gru1_to_pred)
...
class pfc_critic(parl.Model):
    def __init__(self, input_output_info):
        ...
        "从gru到pred"
        self.gru_to_pred=nn.Sequential(
            Conv_Decoder_net()
            # nn.Linear(gru1_hid_dim,np.prod(input_output_info['hpc_img_shape']))
        )
    ...
    def forward(self, input_dict,train,h_dict):
        ...
        'pred'
        _b,_t,_d=gru_out.shape
        criticpred=self.squeeze_time_layer(paddle.reshape(gru_out,[_b,_t,1,16,16]),self.gru_to_pred)

        value_dict={
            'value':value_pred,
            "criticpred":criticpred
            }
        return value_dict
...
class GRUPPO(parl.Algorithm):
    ...
    def learn(self, train_dict):
        ...
        "预测损失"
        pfcpred=action_dict['pfcpred']
        pred_label=train_dict['hpc_img']

        criticpred=value_dict['criticpred']

        pfcpred_loss=paddle.mean((pfcpred-pred_label.reshape(pfcpred.shape))**2)
        criticpred_loss=paddle.mean((criticpred-pred_label.reshape(criticpred.shape))**2)
        pred_loss=pfcpred_loss+criticpred_loss
        # print("pfcpred.shape",pfcpred.shape,pfcpred_label.shape)

        (10.0*pfcpred_loss+0.5*criticpred_loss+value_loss * self.value_loss_coef + action_loss -dist_entropy * self.entropy_coef).backward()

        self.optimizer.step()
        self.optimizer.clear_grad()

        return pred_loss.numpy(),value_loss.numpy(), action_loss.numpy(), dist_entropy.numpy()
...
class GRUPPOAgent(parl.Agent):
    ...
    def sample(self,input_dict,train,h_dict):
        ...
        for key in sample_dict:
            sample_dict[key]=sample_dict[key].detach().numpy()
        return sample_dict
    def reset(self):
    ...
    def learn(self, next_value, gamma, gae_lambda, ppo_epoch, num_mini_batch,
        ...
        for e in range(ppo_epoch):
            ...
            for sample_dict in data_generator:
                ...
                t+=1
                # while 1:
                pred_loss,value_loss, action_loss, dist_entropy = self.alg.learn(train_dict)
        ...
        pred_loss_epoch/=num_updates
        value_loss_epoch /= num_updates
        action_loss_epoch /= num_updates
        dist_entropy_epoch /= num_updates

        return pred_loss_epoch,value_loss_epoch, action_loss_epoch, dist_entropy_epoch
...
class PPO_GRU_Module():
    ...
    def _input(self,input_dict,train,h_dict):
        ...
        output_dict={
            ...
            "action":np.squeeze(sample_dict['action']),
            "action_log_prob":np.squeeze(sample_dict['action_log_probs']),
            "pfcpred":sample_dict['pfcpred'],
    ...
    def _learn(self):
        ...
        pred_loss,value_loss, action_loss, dist_entropy = self.agent.learn(
            next_value, hp.PPO_GAMMA, hp.PPO_GAE_LAMBDA, hp.PPO_EPOCH, hp.PPO_BATCH_SIZE, self.rpm)
        print(f"pred_loss{pred_loss},v_loss{value_loss},a_loss{action_loss},d_entropy{dist_entropy}")
