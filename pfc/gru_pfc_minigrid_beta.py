...
class pfc_actor(parl.Model):
    ...
    def forward(self,input_dict,train,h_dict):
        ...
        action_env_alpha,action_env_beta=action_env_mean_logstd[:,:,:self.action_env_dim],action_env_mean_logstd[:,:,self.action_env_dim:]
        ...
        action_env_alpha=F.sigmoid(action_env_alpha)*4.4+2
        # action_env_logstd=paddle.tanh(action_env_logstd/5)*5
        action_env_beta=F.sigmoid(action_env_beta)*4.4+2
        ...
        action_dict={
            ...
            'action_env_alpha':action_env_alpha,
            'action_env_beta':action_env_beta,
            'action_alpha':action_env_alpha,
            'action_beta':action_env_beta,
            }
...
class GRUPPO(parl.Algorithm):
    ...
    def learn(self, train_dict):
        ...
        alpha,beta=action_dict['action_alpha'],action_dict['action_beta']
        ...
        action_log_probs = dist.log_prob(train_dict['actions_batch']).sum(axis=-1, keepdim=True)

        # print("action_log_probs",action_log_probs.shape)
        # print("actions_batch",train_dict['actions_batch'].shape)
        # print(action_log_probs.numpy()[0].reshape([-1]))
        ...
        # show_log_ratio=(action_log_probs - train_dict['old_action_log_probs_batch']).numpy()
        # for b in range(len(show_log_ratio)):
            
        #     wlog(show_log_ratio[b].reshape([-1]))
        #     wlog(np.min(train_dict['actions_batch'][b].numpy(),axis=-1).reshape([-1]))
        #     wlog(np.max(train_dict['actions_batch'][b].numpy(),axis=-1).reshape([-1]))
        # input()
...
class RolloutStorage(object):
    ...
    def _pad_vec(self,x,max_len,default_value=0.0):
        x_shape=np.shape(x)
        if len(x)==max_len:
            return x
        elif len(x_shape)==1:
            # out=np.zeros([max_len],dtype=x.dtype)
            out=np.full([max_len],fill_value=default_value,dtype=x.dtype)
            out[:len(x)]=x
            return out
        else:
            # print(max_len,x_shape)
            # out=np.zeros([max_len,*x_shape[1:]],dtype=x.dtype)
            out=np.full([max_len,*x_shape[1:]],fill_value=default_value,dtype=x.dtype)
            out[:len(x)]=x
            return out
    ...
    def sample_batch(self,
        ...
        for i in range(num_mini_batch*mini_batch_size):
            ...
            actions_batch=self._pad_vec(self.actions[indices],self.train_history_len*2,default_value=0.5)
