...
"""
class pfc_actor_critic(parl.model):
    def __init__(self,input_output_info) -> None:
        super(pfc_actor_critic,self).__init__()
        ...
        "从gru1到value"
        self.gru1_to_value=nn.Sequential(
            nn.Linear(gru1_hid_dim,128),nl_func(),
            nn.Linear(128,128),nl_func(),
            nn.Linear(128,1)
            )
    ...
    def forward(self,input_dict,train,h_dict):
        ...
        "value"
        value_pred=self.squeeze_time_layer(gru1_out,self.gru1_to_value)
...
class pfc_actor_critic_static(pfc_actor_critic):
    @to_static
    def forward(self,input_dict,train,h_dict):
        return super(pfc_actor_critic_static,self).forward(input_dict,train,h_dict)
...
class GRU_PPO_Model(parl.Model):
    def __init__(self, input_output_info):
        ...
        if input_output_info['to_static']:
            self.actor_critic=pfc_actor_critic_static(input_output_info)
        else:
            self.actor_critic=pfc_actor_critic(input_output_info)
    ...
    def reshape_img(self,img):
        ...
        if len(img_shape)==3:
            ...
            return img
        pass
    def policy_and_value(self, input_dict,train,h_dict):
        tt=timer_tool("model policy")
        action_dict,value_dict=self.actor_model(input_dict,train,h_dict)
        tt.end("all")
        return action_dict,value_dict
...
class GRUPPO(parl.Algorithm):
    ...
    def learn(self, train_dict):
        ...
        def sprint(x,name):
            pass
            # print(name,x.shape)
        action_dict,value_dict = self.model.policy_and_value(train_dict,train=True,h_dict=train_dict)
    ...
    def sample_predict_value(self, input_dict,train,h_dict):
        ...
        action_dict,value_dict=self.model.policy_and_value(input_dict,train,h_dict)
        ...
        predict=action_dict['action_mean']

        value=value_dict['value']

        return [action, action_log_probs],predict,value
...
class GRUPPOAgent(parl.Agent):
    ...
    def sample_predict_value(self, input_dict,train,h_dict):
        action, action_log_probs,predict,value=self.alg.sample_predict_value(self, input_dict,train,h_dict)
        return [action.detach().numpy(),action_log_probs.detach().numpy()],predict.datach().numpy(),value.detach().numpy()
    def reset(self):
        self.alg.model.actor_criric_model.reset()
...
class PPO_GRU_Module():
    ...
    def _input(self,input_dict,train,h_dict):
        ...
        內嗅皮层的未来预测值
        """
        action_and_prob,predict,value=self.agent.sample_predict_value(input_dict,train,h_dict)
        action, action_log_prob=action_and_prob
       
        actor_gru_h1,actor_gru_h2,=self.agent.alg.model.actor_critic.h1.numpy(),self.agent.alg.model.actor_critic.h2.numpy()
        # print(action.shape)
        output_dict={
    ...
    def _learn(self):
        _,_,next_value = self.agent.sample_predict_value({
            ...
            {  
                'actor_init_h1':to_time_paddle(self.rpm.actor_gru1_h[-1],'vec',1,1),
                'actor_init_h2':to_time_paddle(self.rpm.actor_gru2_h[-1],'vec',1,1),
            }
            )
