...
class pfc_actor(parl.Model):
    def __init__(self, input_output_info):
        ...
        self.gru_to_env_action=nn.Sequential(
            ...
            nn.Linear(128,128),nl_func(),
            nn.Linear(128,128),nl_func(),
            nn.Linear(128,self.action_env_dim)
            )
    ...
    def forward(self,input_dict,train,h_dict):
        ...
        action_logits=self.squeeze_time_layer(gru_out,self.gru_to_env_action)
        action_probs=F.softmax(action_logits,axis=-1)
        action_dict={
...
class GRUPPO(parl.Algorithm):
    ...
    def learn(self, train_dict):
        ...
        dist = m_Categorical(logits=probs)
        action=train_dict['actions_batch']
        act_dim = probs.shape[-1]
        actions_onehot = paddle.squeeze(F.one_hot(action, act_dim),axis=-2)
        action_log_probs=paddle.sum(probs*actions_onehot,axis=-1,keepdim=True)
        # print(actions_onehot.shape,action_log_probs.shape)
        # print(probs[0,:10])
        # print(train_dict['model_probs_batch'][0,:10])
        # wlog(f"|probs{probs[0,0].numpy()}|old_probs{train_dict['model_probs_batch'][0,0].numpy()}")
        ...
        # print(paddle.max(ratio*train_dict['train_control']),paddle.min(ratio*train_dict['train_control']))
        ...
        # ratio_in_control=paddle.where(train_dict['train_control']==1.0,ratio,paddle.ones_like(ratio))
        # wlog(
        #     f"|action_log_probs={[paddle.min(action_log_probs).numpy()[0],paddle.max(action_log_probs).numpy()[0]]}"+
        #     f"|old_action_log_probs={[paddle.min(train_dict['old_action_log_probs_batch']).numpy()[0],paddle.max(train_dict['old_action_log_probs_batch']).numpy()[0]]}"+
        #     f"|action_loss={action_loss.numpy()[0]}"+
        #     f"|ratio=[{paddle.min(ratio).numpy()[0]},{paddle.max(ratio).numpy()[0]}]"+
        #     f"|ratio_in_control=[{paddle.min(ratio_in_control).numpy()[0]},{paddle.max(ratio_in_control).numpy()[0]}]"+
        #     f"|adv=[{paddle.min(train_dict['adv_targ']).numpy()[0]},{paddle.max(train_dict['adv_targ']).numpy()[0]}]"
        #     )
        # wlog( 
        #     f"|action_log_prob={action_log_probs[0,:10].numpy()}"+
        #     f"|old_action_log_prob={train_dict['old_action_log_probs_batch'][0,:10].numpy()}"
        #     )
    ...
    def sample(self, input_dict,train,h_dict):
        ...
        # print(actions_onehot.shape,probs.shape)

       


        return value, action, action_log_probs,probs
...
class GRUPPOAgent(parl.Agent):
    ...
    def sample(self,input_dict,train,h_dict):
        ...
        Args:
            obs (np.array): observation
        """
        value, action, action_log_probs,model_probs = self.alg.sample(input_dict,train,h_dict)
        return value.detach().numpy(), action.detach().numpy(), action_log_probs.detach().numpy(),model_probs.detach().numpy()
    def reset(self):
...
class RolloutStorage(object):
    def __init__(self, num_steps, input_output_info):
        ...
        self.actions = np.zeros((num_steps, 1), dtype='int64')
...
class PPO_GRU_Module():
    ...
    def _input(self,input_dict,train,h_dict):
        ...
        內嗅皮层的未来预测值
        """
        value, action, action_log_prob,model_probs=self.agent.sample(input_dict,train,h_dict)
        actor_gru_h,critic_gru_h=self.agent.alg.model.actor_model.h.numpy(),self.agent.alg.model.critic_model.h.numpy()
        # print(action.shape)
        output_dict={
            "value":np.squeeze(value),
            "model_probs":np.squeeze(model_probs),
