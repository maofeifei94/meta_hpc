...
class pfc_actor(parl.Model):
    ...
    def forward(self,input_dict,train,h_dict):
        ...
        action_continuous_mean,action_continuous_logstd=action_continuous_logits[:,:,:self.action_env_continuous_dim],action_continuous_logits[:,:,self.action_env_continuous_dim:]
        action_continuous_mean=paddle.tanh(action_continuous_mean/3)*1.5
        action_continuous_logstd=paddle.log(F.softplus(action_continuous_mean))
...
class GRUPPO(parl.Algorithm):
    ...
    def learn(self, train_dict):
        ...
        action_discrete_surr2=paddle.clip(action_discrete_ratio,1.0-hp.PPO_DISCRETE_CLIP_PARAM,1.0+hp.PPO_DISCRETE_CLIP_PARAM)*adv_targ
        ...
        action_continuous_log_probs=action_continuous_dist.log_prob(old_action_continuous)
        action_continuous_dist_entropy=paddle.mean(action_continuous_dist.entropy().sum(axis=-1,keepdim=True)*train_control)
        ...
        action_continuous_surr2=paddle.clip(action_continuous_ratio,1.0-hp.PPO_CONTINUOUS_CLIP_PARAM,1.0+hp.PPO_CONTINUOUS_CLIP_PARAM)*adv_targ
        action_continuous_loss=-(paddle.minimum(action_continuous_surr1,action_continuous_surr2)*train_control).mean()

        "value loss"
        if hp.PPO_USE_CLIPPED_VALUE_LOSS:
            ...
            # value_limit = train_dict['value_preds_batch'] + (train_dict['returns_batch'] - train_dict['value_preds_batch'])*hp.PPO_CLIPPED_VALUE_RATIO
            # value_loss_limit=0.5 * (train_dict['returns_batch'] - value_limit).pow(2)
            # value_loss_real=0.5 * (train_dict['returns_batch'] - values).pow(2)
            # value_loss_full=0.5 * (train_dict['returns_batch'] - train_dict['value_preds_batch']).pow(2)
            # value_loss=(paddle.maximum(value_loss_limit,value_loss_real)*train_dict['train_control']).mean()
            # value_loss=(0.5 * (value_limit - values).pow(2)*train_dict['train_control']).mean()
            ...
            # value_loss=(paddle.abs(value_update_ratio-hp.PPO_CLIPPED_VALUE_RATIO).pow(2)*paddle.abs(value_delta_target)**2*train_dict['train_control']).mean()

            # value_loss=(paddle.abs(value_delta-value_delta_target*hp.PPO_CLIPPED_VALUE_RATIO)**2*train_control).mean()
            # value_loss=((value_update_ratio-hp.PPO_CLIPPED_VALUE_RATIO)*paddle.abs(value_delta_target)).detach()*values
            # value_loss=(value_loss*train_control).mean()
            value_loss=(paddle.abs((value_delta-value_delta_target*hp.PPO_CLIPPED_VALUE_RATIO)**2/value_delta_target_non_zero)*train_control).mean()

            value_clip_mean_ratio=paddle.sum(paddle.abs(value_update_ratio)*train_control)/paddle.sum(train_control)

            # value_clip_mean_ratio=paddle.abs(values-train_dict['value_preds_batch'])
            # value_clip_mean_ratio/=(paddle.abs(train_dict['returns_batch'] - train_dict['value_preds_batch'])+1e-12)
            # value_clip_mean_ratio=paddle.sum(value_clip_mean_ratio*train_control)/paddle.sum(train_control)
            # print("value_clip_mean_ratio",value_clip_mean_ratio.numpy()[0])
            # print((value_update_ratio*train_control).numpy()[0].reshape([-1]))
            # print(((values-train_dict['value_preds_batch'])*train_control).numpy()[0].reshape([-1]))
            # print((paddle.abs(train_dict['returns_batch'] - values)/(paddle.abs(train_dict['returns_batch'] - train_dict['value_preds_batch'])+1e-12)*train_control).numpy()[0].reshape([-1]))
            # print(paddle.abs(train_dict['returns_batch'] - train_dict['value_preds_batch']).numpy()[0].reshape([-1]))
            # print((paddle.abs(value_update_ratio)*train_control).numpy()[0].reshape([-1]))
            # input()

        else:
            value_loss = (0.5 * (train_dict['returns_batch'] - values).pow(2)*train_dict['train_control']).mean()
            value_clip_mean_ratio=paddle.zeros([1])

        total_loss=value_loss*hp.PPO_VALUE_LOSS_COEF
        total_loss+=(action_continuous_loss-action_continuous_dist_entropy*hp.PPO_CONTINUOUS_ENTROPY_COEF)*hp.PPO_CONTINUOUS_LOSS_RATIO
        total_loss+=(action_discrete_loss-action_discrete_dist_entropy*hp.PPO_DISCRETE_ENTROPY_COEF)*hp.PPO_DISCRETE_LOSS_RATIO
        # total_loss=value_loss*hp.PPO_VALUE_LOSS_COEF+action_discrete_loss*1000
        
        self.optimizer_a.clear_grad()
        self.optimizer_c.clear_grad()
        total_loss.backward()
        # print(paddle.abs(action_discrete_ratio.grad).mean()*1000,paddle.abs(values.grad).mean()*1000)
        self.optimizer_a.step()
        self.optimizer_c.step()


        return value_loss.numpy(), action_continuous_loss.numpy(), action_continuous_dist_entropy.numpy(),action_discrete_loss.numpy(),action_discrete_dist_entropy.numpy(),value_clip_mean_ratio.numpy()
...
class GRUPPOAgent(parl.Agent):
    ...
    def learn(self, next_value, gamma, gae_lambda, ppo_epoch, num_mini_batch,
        ...
        action_continuous_loss_epoch /= num_updates
        action_continuous_dist_entropy_epoch /= num_updates
        action_discrete_loss_epoch /= num_updates
        action_discrete_dist_entropy_epoch /= num_updates
        value_clip_mean_ratio_epoch/=num_updates
