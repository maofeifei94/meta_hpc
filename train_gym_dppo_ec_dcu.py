...
from pfc.hyperparam.dppo_gru_hybrid_dcu_ec_hp import DPPO_Hybrid_HyperParam as hp
...
input_output_info={
    'to_static':False,
    'env_vec_dim':env.observation_space.shape[0]+svaepred_info.gloinfo_gauss_dim*2+svaepred_info.locinfo_gru_dim,
    'obs_dim':env.observation_space.shape[0]+svaepred_info.gloinfo_gauss_dim*2+svaepred_info.locinfo_gru_dim,
...
def dppo_learn_process(queue_dict):
    ...
    'main loop'
    for e in range(0 if hp.GAME_LOAD_MODEL is None else hp.GAME_LOAD_MODEL+1,int(1e16)):
        t1=time.time()
        "update"
...
def dppo_interact_process(queue_dict,process_n):
    ...
    "ec"
    ec=SeqVaePred(svaepred_info)
    ec.eval()
    ec.load_model(hp.ec_model_dir,hp.ec_model_num)
    ec_vec=ec.reset()
    print(ec_vec.shape,env_obs.shape,input_output_info['obs_dim'])
    input_dict={
        "env_vec":paddle.concat([env_obs_to_paddle_vec(ec_vec),env_obs_to_paddle_vec(env_obs)],axis=-1), #ec_vec
    }
    ...
    for e in range(int(1e16)):
        ...
        with paddle.no_grad():
            for i in range(hp.PPO_NUM_STEPS):
                ...
                "ec step"
                next_ec_vec=ec(
                    paddle.to_tensor(np.reshape(env_obs,[1,1,-1]),'float32'),
                    paddle.to_tensor(np.reshape(one_hot(agent_action_discrete,input_output_info['action_env_discrete_dim']),[1,1,-1]),'float32'),
                    paddle.to_tensor(np.reshape([reward],[1,1,-1]),'float32')
                    )
                ...
                "next input step"
                next_input_dict={"env_vec":paddle.concat([env_obs_to_paddle_vec(next_ec_vec),env_obs_to_paddle_vec(next_env_obs)],axis=-1)}
