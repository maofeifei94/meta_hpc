...
class Sparse_Autoencoder(nn.Layer):
    def __init__(self,hid_vec_dim,nl_func=nn.Tanh(),gamma_fast=0.99,gamma_slow=0.999,) -> None:
        ...
        encoder=nn.Sequential(
                    ...
                    Reslayer(nn.Sequential(nn.Linear(encoder_hid_dim,encoder_hid_dim),nl_func)),
                    Reslayer(nn.Sequential(nn.Linear(encoder_hid_dim,encoder_hid_dim),nl_func)),
                    nn.Sequential(nn.Linear(encoder_hid_dim,self.encoder_sparse_dim))
                )
    ...
    def sigmoid_verse(self,y):
        return paddle.log(y/(1-y))
    def sigmoid_dev(self,x):
        return F.sigmoid(x)*(1-F.sigmoid(x))
    def data_fire_kl_layer(self,x_before_sigmoid,ratio,step_size):
        x=F.sigmoid(x_before_sigmoid)
        n=x.shape[-1]

        code1=paddle.mean(x**2,axis=1,keepdim=True)
        code2=paddle.mean((1-x)**2,axis=1,keepdim=True)

        slope1=(ratio/code1-(1-ratio)/(1-code1))*(2*x/n)*self.sigmoid_dev(x_before_sigmoid)
        slope2=((1-ratio)/code2-(ratio)/(1-code2))*(2*(x-1)/n)*self.sigmoid_dev(x_before_sigmoid)

        delta_x_before_sigmoid=(slope1+slope2)*step_size

        return x_before_sigmoid+delta_x_before_sigmoid
    def get_loss_data_fire(self,sparse_hid):
        ...
        return loss_data_fire
    def pred_with_loss(self,hid_code_batch):
        sparse_hid_before_sig=self.encoder(hid_code_batch)
        loss_data_kl_list=[]
        for j in range(5):
            sparse_hid_before_sig=self.data_fire_kl_layer(sparse_hid_before_sig,self.sparse_ratio,10000.0)
            loss_data_kl_list.append(self.get_loss_data_fire(F.sigmoid(sparse_hid_before_sig)).numpy()[0])
        sparse_hid=F.sigmoid(sparse_hid_before_sig)
        ...
        loss_and_pred_dict={
            ...
            "loss_data_kl_list":loss_data_kl_list,
