...
class Sparse_cae(nn.Layer):
    def __init__(self, name_scope=None, dtype="float32"):
        ...
        self.sparse_ae=Sparse_Autoencoder(192)
    ...
    def pred_and_recon(self,img):
        ...
        test_img+=paddle.randn(test_img.shape)*0.02
        test_img=paddle.clip(test_img,0.0,1.0)
        cae_z=self.cae.encode(test_img)
        ...
        "recon_img"
        cae_recon_img=self.cae.decode(paddle.reshape(recon_z,cae_z.shape))
    ...
    def learn(self,train_batch_size):
        ...
        train_img+=paddle.randn(train_img.shape)*0.02
        train_img=paddle.clip(train_img,0.0,1.0)
        ...
        sae_recon_hid_code=sparse_ae_loss_pred["recon_hid_code"]
        sae_loss=sae_loss_recon*1000.0+(sae_loss_data_fire*100+sae_loss_cell_fire*50)
        ...
        output_dict={
            "total_loss":total_loss.numpy()[0],
            "cae_loss":[cae_loss.numpy()[0],cae_loss_recon.numpy()[0]],
            "sae_loss":[sae_loss.numpy()[0],sae_loss_recon.numpy()[0],sae_loss_data_fire.numpy()[0],sparse_ae_loss_pred["loss_data_kl_list"],sae_loss_cell_fire_current.numpy()[0],sae_loss_cell_fire_fast.numpy()[0],sae_loss_cell_fire_slow.numpy()[0]],
            "cae_recon_img":cae_recon_img,
            "train_img":train_img
