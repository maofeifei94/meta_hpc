...
class mnist_reader():
    def __init__(self,root="../data/mnist"):
        train_data = torchvision.datasets.MNIST(root=root, train=True,
                                                transform=torchvision.transforms.ToTensor(), download=True)
        train_loader = torch.utils.data.DataLoader(train_data, batch_size=1000, shuffle=True)

        self.train_all_data_label=list(train_loader)

        self.train_all_data=torch.cat([td[0] for td in self.train_all_data_label],dim=0)
        self.train_all_label=torch.cat([td[1] for td in self.train_all_data_label],dim=0)
        print(self.train_all_data.shape)
        print(self.train_all_label.shape)
    def get_batch_data_label(self,batch_size,flatten=True,to_cuda=True):
        rand_index=torch.randint(0,self.train_all_data.shape[0],[batch_size])
        rand_data=self.train_all_data[rand_index]
        rand_label=self.train_all_label[rand_index]

        if flatten:
            rand_data=torch.reshape(rand_data,[batch_size,-1])
        if to_cuda:
            rand_data=rand_data.cuda()
            rand_label=rand_label.cuda()
        return rand_data,rand_label
    def save_data(self):
        print("data_len",len(self.train_all_data))
        ...
        for i in range(len(self.train_all_data)):
            single_img=self.train_all_data[i].data.cpu().numpy()[0]
            single_label=self.train_all_label[i].data.cpu().numpy()
            single_img=(single_img*255).astype(np.uint8)

            save_path=save_dir+"/{}.png".format(i)
            cv2.imwrite(save_path,single_img)

            # print(np.shape(single_img),single_label)

            num_list[single_label]+=1
            print(num_list)
            if i>200:
                break
...
class hippocampus_sparse_autoencoder():
    def __init__(self,data_dim,
                 ...
                 lr = 0.00001,gamma_fire=0.9999,gamma_fire_fast=0.99,use_cuda=True
                 ):
        "hyper parameter"
        self.batch_size=64
        ...
        self.hid_neuron_avg_fire=to_cuda(self.hid_neuron_avg_fire,use_cuda)
        ...
        self.hid_neuron_avg_fire_fast=to_cuda(self.hid_neuron_avg_fire_fast,use_cuda)
        ...
        "cuda"
        self.encoder=to_cuda(self.encoder,use_cuda)
        self.decoder = to_cuda(self.decoder,use_cuda)
        "optimizer"
    ...
    def show_hid(self,hid,name):
        ...
        cv2.imshow(name,img)
    def pred_with_loss(self,data_torch,get_engram=False):
        hid_code = self.encoder(data_torch)
        ...
        loss_top_k = torch.mean(torch.abs(torch.sum(hid_code ** 2, dim=-1, keepdim=True) - self.k) +
                                torch.abs(
                                    torch.sum((1 - hid_code) ** 2, dim=-1, keepdim=True) - (self.sparse_dim - self.k)))
        ...
        if get_engram:
            return output, loss_top_k, loss_restore, loss_KL,hid_code
        else:
            return output,loss_top_k,loss_restore,loss_KL
