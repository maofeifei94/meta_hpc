...
class multi_circle_env():
    ...
    def get_obs(self):
        ...
        all_obs["ball_obs"]=cv2.resize(cv_img,(64,64),interpolation=cv2.INTER_AREA)
    ...
    def render(self):
        ...
        # print("self cv cost",total_cv_time)
        obs=self.obs
        cv2.imshow('ballobs',cv2.resize(obs["ball_obs"],dsize=(256,256),interpolation=cv2.INTER_AREA))
...
if __name__=="__main__":
    ...
    for i in range(100000):
        ...
        obs,reward,done=e.step(action,ask_place=np.array([0.5,0.5]))
        if done:
            print("done")
            e.reset()
