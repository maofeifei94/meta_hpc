...
class multi_circle_env():
    def __init__(self,show=False):
        "[width,height]"
        self.grid_size=np.array([5,5],dtype=np.int)
        self.block_size=np.array([120,120],dtype=np.int)
    ...
    def generate_rand_maze(self):
        ...
        for i in range(goal_num):
            while 1:
                goal_site=list(np.random.randint(0,self.grid_size[0],[2]))
    ...
    def reset(self,):
        ...
        "warfrog"
        self.warfrog_mask=np.full([64,64],-1,np.float32)
    ...
    def step(self,action,ask_place=None,hpc_train_mode=False):
        ...
        if ask_place is not None:
            self._apply_place_to_ball(ask_place)
        else:
            self._apply_action_to_ball(action)
    ...
    def get_obs(self):
        ...
        # all_obs["full_img"]=self.render_full_img_cv()
        ...
        # hpc_img=self.render_and_update_warfrog(ball_site)
        # all_obs['hpc_img']=np.expand_dims(hpc_img,axis=-1)
        ask_ball_obs=self.render_img_cv(np.array(self.ask_place,dtype=np.float32)*600)
        all_obs["ask_ball_obs"]=np.array(cv2.resize(ask_ball_obs,(64,64),interpolation=cv2.INTER_AREA),dtype=np.float32)/255
    ...
    def render_label(self,ball_center):
        ...
        return label_img
    def render_and_update_warfrog(self,ball_center):
        block_site=(ball_center/self.block_size*3).astype(np.int32)
        site_range=2*3
        minx,miny,maxx,maxy=max(block_site[0]-site_range,0),max(block_site[1]-site_range,0),min(block_site[0]+site_range+1,60),min(block_site[1]+site_range+1,60)
        ...
        1:视野内
        """      
        self.warfrog_mask[miny:maxy,minx:maxx]=1

        label_img=np.full([64,64],0,np.float32)
        for goal_site,goal_flag in zip(self.goal_site_list,self.goal_flag_list):
            ...
            label_img[goal_site[0]*3:(goal_site[0]+1)*3,goal_site[1]*3:(goal_site[1]+1)*3]=color

        label_img[block_site[1],block_site[0]]=0.5

        warfrog_img=np.where(self.warfrog_mask>=0,label_img,np.full([64,64],-1))
        self.warfrog_mask[miny:maxy,minx:maxx]=0
    ...
    def render(self):
        ...
        cv2.rectangle(cv_img,(int(ball_site[0])-window_size//2,int(ball_site[1])-window_size//2),(int(ball_site[0])+window_size//2,int(ball_site[1])+window_size//2),(200,50,50))

        ask_place=(self.ask_place*600).astype(np.int32)
        cv2.rectangle(cv_img,(int(ask_place[0])-window_size//2,int(ask_place[1])-window_size//2),(int(ask_place[0])+window_size//2,int(ask_place[1])+window_size//2),(200,50,150))
        # total_cv_time+=time.time()-t1
        # print("self cv cost",total_cv_time)
        obs=self.obs
        cv2.imshow('askballobs',cv2.resize(obs["ask_ball_obs"],dsize=(256,256),interpolation=cv2.INTER_AREA))
        # cv2.imshow('self_cv',cv2.resize(obs["self_cv"],dsize=(256,256)))
        # cv2.imshow('labelimg',cv2.resize(obs['label_img'],dsize=(256,256),interpolation=cv2.INTER_AREA))
        # cv2.imshow('hpc_img',cv2.resize((obs['hpc_img']+1)/2,dsize=(256,256),interpolation=cv2.INTER_AREA))
...
if __name__=="__main__":
    ...
    for i in range(100000):
        ...
        obs,reward,done=e.step(action,ask_place=np.array([0.3,0.3])*(i%5),hpc_train_mode=True)
        # cv2.imshow('ballobs',cv2.resize(obs["ball_obs"],dsize=(256,256)))
        # # cv2.imshow('self_cv',cv2.resize(obs["self_cv"],dsize=(256,256)))
        # cv2.imshow('labelimg',cv2.resize(obs['label_img'],dsize=(256,256)))
        # cv2.imshow('hpc_img',cv2.resize((obs['hpc_img']+1)/2,dsize=(256,256)))
        e.render()
