...
class multi_circle_env():
    ...
    def generate_rand_maze(self):
        ...
        for i in range(goal_num):
            while 1:
                ...
                if goal_site in wall_list or goal_site in goal_site_list:
                    continue
                else:
                    break
        ...
        self.wall_list,self.door_list,self.goal_site_list=wall_list,door_list,goal_site_list,
    ...
    def save_maze_param(self,maze_name,save_dir="maze_param"):
        maze_param=[self.wall_list,self.door_list,self.goal_site_list,self.real_goal_index_list,self.goal_flag_list]
        # now_time=time.localtime(time.time())
        npy_path=f"{save_dir}/maze_{maze_name}.npy"
        np.save(npy_path,maze_param)
        return npy_path
    def load_maze_param(self,file_path):
        self.wall_list,self.door_list,self.goal_site_list,self.real_goal_index_list,self.goal_flag_list=np.load(file_path,allow_pickle=True)
    ...
    def reset(self,):
        ...
        while 1:
            ...
            if ball_site in self.wall_list or ball_site in self.goal_site_list:
                continue
            else:
        ...
        obs=self.get_obs()
        return obs
    def step(self,action,place=None,hpc_train_mode=False):
        ...
        # self._apply_place_to_ball([0,0])
        # print("ball_site1=",self._get_position(self.ball))
        self.physics_run()
        self.steps_num+=1
    ...
    def get_obs(self):
        ...
        all_obs["ball_site_norm"]=ball_site_norm

        goal_site_list=self._get_goal_site()
        all_obs["goal_site_list"]=goal_site_list
        goal_site_norm_list=[np.array(goal_site)/self.img_size for goal_site in goal_site_list]
        all_obs["goal_site_norm_list"]=goal_site_norm_list

        real_goal_site=[goal_site_list[rg] for rg in self.real_goal_index_list]
        all_obs["real_goal_site"]=real_goal_site
        real_goal_site_norm=[goal_site_norm_list[rg] for rg in self.real_goal_index_list]
        all_obs["real_goal_site_norm"]=real_goal_site_norm

        cv_img=self.render_img(ball_site)
...
if __name__=="__main__":
    ...
    for i in range(100000):
        obs,reward,done=e.step(np.random.uniform(-1,1,[2]),hpc_train_mode=True)
        print(reward)
        cv2.imshow('ballobs',obs["ball_obs"])
        cv2.waitKey()
