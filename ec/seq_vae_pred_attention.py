...
class SeqVaePred(nn.Layer,ModelSaver):
    def __init__(self,info:svaepred_info):
        ...
        self.gloinfo_gru=HierarchicalGru(self.info.gloinfo_ifc_output_dim,self.info.gloinfo_gru_dim,4,4)
    ...
    def _cal_pred(self,gauss_sample,local_h):
        ...
        Qmat=self.btd_layer(self.pred_Qnet,local_h).reshape([_b,_t,1,self.info.Q_dim])
        Kmat=self.btd_layer(self.pred_Knet,gauss_sample).reshape([_b,1,self.info.Q_dim,self.info.V_num])
        Kmat=paddle.tile(Kmat,[1,_t,1,1])
        Vmat=self.btd_layer(self.pred_Vnet,gauss_sample).reshape([_b,1,self.info.V_num,self.info.V_dim])
        Vmat=paddle.tile(Vmat,[1,_t,1,1])
